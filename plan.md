...
Test Plan: Frenchify (v1.5)Project Name: FrenchifyVersion: 1.0Scope: Functional, UI/UX, Security, and Integration Testing for the MVP.Test Strategy: Manual testing on Chrome/Firefox (Desktop & Mobile) + Unit testing for Cloud Functions.1. Authentication & Onboarding TestsObjective: Verify secure access and correct default role assignment.IDTest CaseStepsExpected ResultSourceAUTH-01Student Signup (Default Tier)1. Navigate to Signup page.2. Register with email/password.3. Check Firestore users collection.User is created with role: "student" and tier: "free". AUTH-02Teacher Signup (Pending State)1. Register as a Teacher.2. Log in immediately.3. Attempt to "Publish" a course.Login succeeds, but "Publish" button is disabled/hidden. Status is "pending". AUTH-03Unified Login1. Log in with a Free Student account.2. Log in with a Paid Student account.Both use the same login form successfully. AUTH-04Simultaneous Login Check1. Log in on Browser A.2. Log in on Browser B (same account).3. Refresh Browser A.(MVP Scope) Either a warning banner appears, or the user is allowed (depending on final implementation decision). 2. Student Functional TestsObjective: Ensure the "Freemium" logic works perfectly (Netflix-style access).IDTest CaseStepsExpected ResultSourceSTU-01Free Tier - Access Free Content1. Login as Free Tier.2. Click a course marked "Free".3. Open a material where isFreePreview = true.Content loads and plays correctly. STU-02Free Tier - Block Paid Content1. Login as Free Tier.2. Click a course marked "Paid".Course card shows a "Lock" icon. Clicking triggers the "Upgrade" modal. STU-03Paid Tier - Global Access1. Login as Paid Tier.2. Click any Paid course.3. Click any locked material.Immediate access granted (No individual purchase flow). STU-04Paid Tier - Download1. Login as Paid Tier.2. Open a PDF material.3. Look for Download button.Download button is visible and functional. STU-05Free Tier - No Download1. Login as Free Tier.2. Open the same PDF material.Download button is hidden or disabled. STU-06Paid Tier - Ask Question1. Login as Paid Tier.2. Navigate to a video lesson.3. Type and submit a question.Question appears in the Q&A list. Stored in questions sub-collection. 3. Teacher Functional TestsObjective: Verify content management and YouTube integration.IDTest CaseStepsExpected ResultSourceTCH-01Create Course (Draft)1. Login as Teacher.2. Create a new course.3. Check Firestore courses collection.Course created with status: "draft". TCH-02Embed YouTube (Grammar)1. Add Material -> Select "YouTube".2. Paste a valid YouTube URL.3. Save.System extracts Video ID. Student view renders the YouTube player, not a file player. TCH-03Upload Audio (Conversation)1. Add Material -> Select "Video/Audio".2. Drag & drop an MP3 file.3. Save.File uploads to Firebase Storage. URL saved to Firestore. TCH-04Create Quiz (Client-Side)1. Add Material -> Select "Quiz".2. Add 3 questions.Quiz saves. Note: Verify no gradebook/results table is created in DB. TCH-05Reply to Question1. Go to Q&A Dashboard.2. Find unanswered student question.3. Submit reply.Reply is saved (answerText) and displayed to the student. TCH-06Publish Course (Approved)1. Ensure Teacher is Approved.2. Click "Publish" on a draft.Status updates to published. Course appears in Student Catalog. 4. Admin Functional TestsObjective: Verify gatekeeping and user management.IDTest CaseStepsExpected ResultSourceADM-01Teacher Approval1. Login as Admin.2. Locate "Pending Teacher" in dashboard.3. Click "Approve".Teacher can now see the "Publish" button on their dashboard. ADM-02Teacher Rejection1. Click "Reject" on a pending teacher.Teacher status updates (or account deleted, depending on logic). Cannot publish. ADM-03Subscription Override1. Find a Free Student.2. Manually toggle tier to "Paid".Student immediately gains access to Paid content (simulating support fix). ADM-04Delete User1. Find a user.2. Click "Delete".User removed from Authentication AND Firestore users collection. 5. Security & Technical TestsObjective: Verify Firestore Rules and Data Integrity.IDTest CaseStepsExpected ResultSourceSEC-01Free Tier Data Leak1. Log in as Free Tier.2. Use browser console to try reading a material document where isFreePreview = false.Firestore returns permission-denied error. SEC-02Paid Tier Data Access1. Log in as Paid Tier.2. Try reading the same restricted document.Firestore allows the read. SEC-03Cross-Teacher Write1. Log in as Teacher A.2. Try to edit a course owned by Teacher B via API/Console.Firestore returns permission-denied. Only owner can edit. SEC-04Custom Claims Check1. Upgrade a user.2. Check their ID Token.Token contains tier: 'paid' claim (to reduce DB reads). 6. UI/UX VerificationObjective: Ensure the "Frenchify" brand guidelines are met.IDTest CaseStepsExpected ResultSourceUI-01Visual Identity1. Inspect buttons and headers.Primary buttons are Azure Blue (#2563EB). "Pro" badges are Gold (#F59E0B). UI-02Course Card Logic1. View Catalog as Free Student.Paid courses have a grayscale filter and a lock icon overlay. UI-03Responsive Layout1. Open Student Dashboard on Mobile (375px width).Grid collapses to single column. Navbar adapts.